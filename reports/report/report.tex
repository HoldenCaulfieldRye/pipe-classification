...
clampdet, same weights, freeze backprop up to and including: 
     -> none:  clampdet/08
     -> conv1: 
     -> conv2: 
     -> conv3: 
...
thresh
threshold_freeze{5.5,6,7}/logs/11-08-2014) 


keepers:
hatch markings: 03, 15
clampdet: thresh, thresh_freeze6/11, thresh_freeze7/11
...
Motivation:
'CNN features off the shelf' advocates training a linear SVM on top of the transferred feature space. The difference with a standard deep neural network are:
- linearity
- optimise margin-based loss rather than log probability of the data
{cite arxiv.org/pdf/1306.0239.pdf}
By maximising log prob of the data, the deep neural net learns models of the probability distributions that produce both classes. The SVM on the other hand merely chooses the discriminative frontier that maximises the space between the support vectors, i.e. that leaves most space between the two classes. \\

Softmax performs a logistic regression: http://ufldl.stanford.edu/wiki/index.php/Softmax_Regression
...
Results:
Comment in light of http://metaoptimize.com/qa/questions/13253/least-squares-vs-cross-entropy-vs-hinge-loss
and of http://arxiv.org/pdf/1306.0239v1.pdf
...
Four approaches were taken to tackle class imbalance, the first ones being simplest to implement, the last ones most sophisticated and information-preserving. \\

A fixed architecture is kept throughout the experiments: backprop on all layers, fc weights re-initialised. \\
...
This was trained with cuda-convnet. \\
...
This presents best results (accuracy overall, on pos, on neg, sig level) for each binary classifier and discusses further improvements. This is where you mention the multi-query issue, the mis-labelling issue. \\

Also mention the things you could not experiment with: the entire list in your .md file. \\
