...
Thanks to Razvan Ranca for wonderful strategic advice on which experiments to choose for the exploration of hypotheses, help with C++, musical and caffeine tricks for staying up all night, and much else. I look excitingly forward to the times ahead cofounding a machine learning startup with him!
...
Thanks to the people on the Google+ Deep Learning community who provided helpful comments and guidance; especially Dr Soumith Chintala on transfer learning and Prof Francisco Zamora-Martinez on class imbalance.
...
Deep Convolutional Neural Networks reA most complete and concise review of ConvNet architecture can be found in "Stochastic Pooling" paper. You could imitate the structure and expound on every single point with image examples. \\
...
\subsubsection{Operations in each layer}
...
\paragraph{Filter aka Pixel Feature}
...
Differentiable, symmetric, bounded activation functions that introduce a non-linearity smoothly over the entire input domain were the activation units of choice in deep feed-forward neural networks, until Rectified Linear Units were used for CNNs in 2012 \cite{krizhevsky} and found to outperform their counterparts:

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.4]{images/ReLU_train_fast.png}
	\caption{ReLUs in CNNs train faster (Krizhevsky et al 2012)}
\end{figure}
...
\item the function is non-linear only in the neighbourhood of $0$ (note that the motivation for learning non-linear functions is the range of functions they can learn; linear functions are greatly limited in this respect, e.g.\ no boolean linear function can represent XOR \cite{univ-approx}).
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.8]{images/activation_functions.png}
	\caption{ReLUs in CNNs train faster (Krizhevsky et al 2012)}
\end{figure}

These points provide motivation to seek a mathematical explanation for these surprising results. In the context of this project, the motivation is to understand the mechanics of training for the CNNs to be used. \\

\subsection{Literature Review}

The literature currently does not provide a mathematical explanation for the superiority of ReLU over sigmoid and tanh units in deep neural networks trained with backpropagation. 

"Rectified linear units improve Restricted Boltzmann Machines" by Nair and Hinton (2010) uses a noisy version of them ($f(x) = max(0, x + N(0, \sigma(x)))$) to replace binary stochastic neurons in Restricted Boltzmann Machines. Detailed mathematical justifications are provided along with experimental evidence, but the analysis focuses on the ability of ReLUs to replace binary stochastic neurons (which are very different to sigmoid and tanh as seen previously), in the task of training Restrictied Boltzmann Machines, which are trained with the unsupervised Contrastive Divergence learning algorithm, not backpropagation \cite{ReLU_RBM}. \\ 

"ImageNet classification with deep convolutional neural networks" by Krizhevsky, Sutskever and Hinton (2012) \cite{krizhevsky} are the first to notice the benefits of ReLU in the context of supervised deep learning (i.e.\ backpropagation). They make the observations that $f(x) = tanh(x)$ or $f(x) = (1+ exp^{-x})^{-1}$ "saturating non-linearities" are slower to train than the $f(x) = max(0,x)$ "non-saturating non-linearity", and report an "accelerated ability to fit the training set", but do not provide any explanations. 

"Deep sparse rectifier networks" by Glorot, Bordes and Bengio (2013) focus on the performance of deep neural networks with ReLU activations by running experiments on supervised (and unsupervised) tasks, varied datasets, and comparing performance with models using $tanh$ as activations. 
...
\item \textbf{Distributed but sparse}: having multiple 
\end{enumerate} 

These reasons are not verified by experiments that specifically track each given characteristic and its impact on training speed or performance. 

Additionally, the authors address the fourth point raised  in the motivations above by explaining that the upward unboundedness of the ReLU can be controlled with an $L_1$ regulariser:  FILL IN ! 

"The only non-linearity in the network comes from the path selection associated with individual neurons being fired or not".


\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.28]{images/ReLU_path_selection.png}
	\caption{ReLU path selection (source: Glorot et al 2013)}
\end{figure}


However, they provide no mathematical explanation for the key observation in Krizhevsky et al 2012 that ReLUs $accelerate training$. However, they 

Note that (Glorot et al 2013) also empirically establishes that "rectifying activation allows deep networks to achieve their best performance without unsupervised pre-training" given sufficient labelled training data. This has probably played an important part in the shift away from unsupervised pre-training to initialise the weights of a network, which would otherwise have been considered in the later stages of this project. \\

observe that ... explain that ...
""


\subsection{Proposed Mathematical Explanation}
