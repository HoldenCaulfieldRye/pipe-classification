...
\begin{equation}
\mathcal{C}_hinge(\textbf{w}, b) = \sum \limits_{i=1}^{n} max(0, 1 - t_i(\textbf{w}^T \phi(\textbf{x}) + b)) + \lambda ||\textbf{w}||^2
\end{equation}

Note that the cost function is denotes as a function of the weights and bias, since these are the parameters that are allowed to vary in order to minimise the expression. The $L_2$ regularisation component is $\lambda ||\textbf{w}||^2$, which penalises high weight values in order to reduce overfitting. The other component is the hinge loss itself, which can understood by bearing in mind that with binary SVMs, target label values are \{1, -1\}.

A softmax output layer with two output units performs a logistic regression \cite{ML-book} i.e.\ the minimisation of the cross entropy error:

\begin{equation}
\mathcal{C}_{CE}(\textbf{w}, b) = \frac{1}{n}\sum \limits_{i=1}^{n} t_i \log(\textbf{w}^T \phi(\textbf{x}) + b) + (1-t_i)\log(1-\textbf{w}^T \phi(\textbf{x}) + b)
\end{equation}

 In order to compare it with the linear SVM, the regression must be recast with \{1, -1\} target labels rather than \{0, 1\} that is conventional for logistic regression. 
...
- optimise \{1, -1\}margin-based loss rather than log probability of the data
