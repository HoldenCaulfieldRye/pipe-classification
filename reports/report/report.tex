...
\subparagraph{MSE} 
\subparagraph{Cross-Entropy for Softmax}

\paragraph{Mini-batch Training for Noisy Gradient Estimates}

Mini-batch training deals with the noisy gradient estimates that are obtained if one considers the learning task as stochastic: data is sampled with noise, a sample of class instances must be averaged to glean class-level information from it. Explain stochastic gradient descent with some fancy statistics (unbiased, consistent estimator).

\paragraph{Decaying Learning Rate for Convergence}


\paragraph{Cross Validation for Generalisation}
...
\paragraph{Dropout for Generalisation}


\subsubsection{Local vs Global Optimisation}

Random matrix theory for pseudo-global optimisation of CNNs with randomised weights!
