...
A number of significant challenges arise from this task: multi-tagging, semantic complexity, domain change, small dataset size and class imbalance. Before going into them, an overview of the data is given below. \\
...
\begin{figure}
    ...
    \begin{minipage}[b]{\textwidth}
      ...
      \begin{subfigure}{.5\textwidth} 
        ...
        % ...here to lower the subcaptions and...
        \caption{Transfer learning, ender-sampling to 90\% imbalance}\label{fig:2b}
      \end{subfigure} \par \vspace*{20pt} % ...remove them from here
      \caption{Bayesian Cross Entropy by itself}\label{fig:2}
    \end{minipage}%
\end{figure}

This is a benchmark against which to evaluate Bayesian cross entropy for the soil contamination task. Both models shown have AlexNet architecture. Under-sampling is successful once more, though not as much as with the clamp detection task: it provides 8 a
...
Lowering the learning rate from 0.0001 to 0.00001 does wonders. The amplitude of the validation error reduces, the error ceases to oscillate, and a steady downward trend can be observed. This indicates that Bayesian softmax cross entropy by itself on AleNet architecture works best, without removing a pooling layer and a fully connected layer. The cost function delivers remarkable gains, since adding it alone provides 18 additional percentage points in average per class accuracy. Maximum per class accuracy is 78\%, i.e.\ 18 which is truly impressive considering how hard this task his - let us recall that transfer learning barely delivered any gains. Furthermore, learning is halted after 2000 iterations (this was due to time constraints), before overfitting begins, so it seems clear from the plots that more progress can be made by resuming training. \\

A potential explanation for why the gains from the Bayesian cross entropy are even greater for this task than for clamp detection wit 98\% imbalance is because imbalance is not as stark in this task, so the variance of the modulus of the gradient is lower, i.e.\ the gradient is less noisy. Since this issue could be addressed by ensuring equal class distribution at every mini-batch, the size of gains observed here could also potentially be seen with clampdet detection 98\% and other tasks with similar imbalances. \\
...
\begin{table}[h!]
   \centering
    ...
    Hatch Markings      & ~ 3,793  & ~     & ~    & ~ 77.4\%  & ~ 42\%    \\
    Insertion Depth     & ~ 897    & ~     & ~    & ~ 86.3\%  & ~ 65\%      \\
    Soil Contam         & ~ 532    & ~ X   & ~ X  & ~ 78.1\%+ & ~ 43\%+      \\
    Water Contam        & ~ 312    & ~ X   & ~ X  & ~ 50\%    & ~ 0\%    \\
    Scrape Zones        & ~ 2,545  & ~ X   & ~    & ~ 61.6\%  & ~ 18.5\%     \\
