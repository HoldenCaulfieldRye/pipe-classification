...
The literature on deep learning with class imbalance was found to be scarce. `Training cost sensitive neural networks with methods addressing the class imbalance problem' by Zhou and Liu, 2005 \cite{zhou}, and `Learning when data sets are imbalanced and when costs are unequal and unknown' by Maloof, 2003 \cite{maloof}, both cover \textbf{under-sampling}, textbf{over-sampling} and \textbf{threshold-moving}. Both find threshold-moving to perform best. Similar approaches to these three techniques were implemented on the clamp detection task and are later discussed in the experimentation subsection. 

(Maloof 2003) explores five classifiers including the perceptron, finding that it distinguishes itself from the other classifiers by `learning to always predict the majority class'. This coincides with the behaviour of the multi-layer perceptrons used during this project, and suggests that the perceptron learning rule is one of the most sensitive learning algorithms to class imbalance. \\

`F-measure as the error function to train neural networks' by Pastor-Pellicer et al, 2013 \cite{f-measure}, does not make use of these techniques and instead introduces a cost function for binary classifiers which is the harmonic mean of precision and recall. Deep neural networks are trained on it as well as on the Mean Squared Error for 3 image-cleaning tasks with up to 95.4\% class imbalance. The image-cleaning task is framed as a classification task where the probability of ink in each pixel of the clean image is to be predicted given the noisy image. MSE as a cost function for training deep neural network classifiers is known to train slower and converge to the same performance as when the cross entropy error is used \cite{DL-book}. The authors find that `both training techniques perform quite well' on the basis that `a well cleaned image gives good results on both metrics'. This seems to merely state that the error computed on a correctly classified input is low in absolute terms, for both cost functions. An analytical comparison of the cost functions by rescaling input domain and output range, such as the one between softmax cross entropy and hinge loss in (Bishop 2010) \cite{ML-book} and in the previous section, is not included. An empirical comparison of the cost functions by evaluating a model trained on each against a common performance metric such as percentage classification accuracy is not provided either. It appears that this cost function does not deliver significant performance gains, and was therefore not implemented for this project.
...
Under-sampling rebalances the training set by removing majority class examples. The brute force approach is to randomly remove these cases and train on the remaining set. However, (Zhou and Liu 2005) \cite{zhou} describe an algorithm for removing selectively. It is reformulated below.
...
\item While there remains examples set aside:
	...
	\item If training set remains unbalanced, randomly randomly remove non minority class examples.
...
at every iteration:
a) 
b) don't train yet but classify all new arrivals. remove from training set those correctly classified.
c) if training set not perfectly balanced, remove non minority class cases who have a nearest neighbour of different class.
d) if training set not perfectly balanced, rebalance by random deletion.
...
\bibitem{coursera}
 ...
 Coursera,
 URL: \url{http://coursera.com/neuralnets}
