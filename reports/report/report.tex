...
1. theory
- mathematical intuition for ReLU vs sigmoid activation function
2. empirics
...
when n is optimal, we have t = n^*/(N-n^*)
<=> N*t - t * n^* = n^*
<=> N*t = n^* *(t+1)
<=> n^* = N*t/(t+1) \\
...
\paragraph{Use More Info}

Instead of trying to predict how to decrease the error rate, why not use the info that gets generated as training develops? i.e. look back and decrease error rate if it's very volatile. \\
...
A binary classifier for each flag. Easy to implement, and maintains independence between the flags (which is prevents 'unfair' bias/discrimination that is present with humans! So improvement towards superhuman performance.\\
