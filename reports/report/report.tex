...
`CNN features off the shelf' by Razavian et al \cite{off-the-shelf} established state-of-the-art results by training linear SVMs on the features from the 7th, fully connected layer of OverFeat. One may wonder why `CNN features off the shelf' trains a linear SVM rather than a conventional softmax layer. What follows is a mathematical comparison of the two \footnote{Heavily drawn from (Bishop 2010)}. 
...
Since logistic regression has range [0, 1] but linear SVM has range [-1, 1], we need to rescale logistic regression output values for the ranges to match. This can be done by taking $\psi_2(\textbf{x}) = t_i \psi(\textbf{x}_i)$ for the ranges to match, and for the two associated cost functions to be comparable. The maximum likelihood estimates of the parameters with logistic regression are obtained by minimising the negative of the log-likelihood function over the training set (i.e.\ the cross entropy), which when reformulated with SVM target labels is given by: 

\begin{equation}
\mathcal{C}_{CE}(\textbf{w}, b) = \frac{1}{n} \sum\limits_{i=1}^{n}\log(1 + \exp(-t_i(\textbf{w}^T \phi(\textbf{x}) + b)))
\end{equation}

The graphical representations below show both error functions for a single training case. 
...
Therefore, the hinge loss encourages sparser output probabilities, which can be understood as more confidence as to which class an input belongs to. Note that this could be considered as `over-confidence', since logistic regression produces a Maximum Likelihood Estimation of the parameters; therefore its output probabilities are the most likely i.e.\ `truest' values, based on the probability distributions inferred from the training set. \\

This experimental section consisted in reproducing the results of (Razavian et al 2014)\footnote{with the exception that the transfer model used here is not OverFeat, but the slightly lesser performing Caffe Reference ImageNet model.} to use them as a benchmark against the optimisations and results produced during this research project.
