...
Notice that this is equivalent to the vector product component of the generic neural network combination of inputs $z = b + \sum\limits_{i=1}^n x_{i}\cdot w_{i}$ where the matrices are flattened into a vector and every weight corresponding to a pixel that is not part of the window is fixed at zero. Fixing many weights to zero imposes a strong prior on the network and significantly reduces the function space, making it easier to search for good functions. \\

By convolving the kernel over the image, one obtains a feature response map (also referred to kernel map):
...
Another advantage of such pixel features is that one can make sense of what the weights represent i.e.\ what the network is learning. For example, consider the feature activation response of convolving the following kernel on the following image: 

%\begin{figure}[!htb]
%\minipage{0.32\textwidth}
%  \includegraphics[width=\linewidth]{}
%  \caption{Image}\label{fig:2a}
%\endminipage\hfill
%\minipage{0.32\textwidth}
%  \includegraphics[width=\linewidth]{}
%  \caption{Kernel}\label{fig:2b}
%\endminipage\hfill
%\minipage{0.32\textwidth}%
%  \includegraphics[width=\linewidth]{}
%  \caption{Kernel Map}\label{fig:2c}
%\endminipage
%\end{figure}
...
\begin{figure}[h]       
    ...
    \hspace{30px}
    \fbox{\includegraphics[scale=0.25]{images/kernel_edge_enhance.png}}
    \hspace{30px}
...
%\centering
%\subfigure[text]{
%\includegraphics[width=.225\textwidth]{.jpg}
%}
%\subfigure[text]{
%\includegraphics[width=.225\textwidth]{.png}
%}
%\subfigure[text]{
%\includegraphics[width=.225\textwidth]{.png}
%}
...
Note that this subsection on explaining sliding kernels is heavily based on a blog post by Christopher Colah \cite{colah}.
...
Note this subsection on explaining the operations in a convolutional layer is heavily based Robert Fergus's NIPS 2013 tutorial \cite{}
