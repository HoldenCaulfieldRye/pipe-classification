...
\subsubsection{Operations in a convolutional layer}
...
\paragraph{Sliding Kernel a.k.a\ Sliding Filter a.k.a\ Pixel Feature}
...
\begin{figure}[h!]
	...
	\caption{source: Krizhevsky et al 2012}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.7]{images/activation_functions.png}
	\caption{}
\end{figure}
...
(Nair and Hinton 2010) \cite{ReLU_RBM} use a noisy version of them ($f(x) = max(0, x + N(0, \sigma(x)))$) to replace binary stochastic neurons in Restricted Boltzmann Machines. Detailed mathematical justifications are provided along with experimental evidence, but the analysis focuses on the ability of ReLUs to replace binary stochastic neurons (which are very different to sigmoid and tanh as seen previously), in the task of training Restricted Boltzmann Machines, which are trained with the Contrastive Divergence algorithm. This differs markedly from the ability of ReLUs to replace tanh neurons in the task of training convolutional neural networks with backpropagation \cite{ReLU_RBM}. \\ 
