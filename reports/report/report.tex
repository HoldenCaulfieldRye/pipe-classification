...
The bulk of research undertaken to make more general improvements in understanding deep learning and dealing with class imbalance, combined with time constraints, have restricted the time spent optimising each one of the 10 tasks. For the moment, average per class classification accuracies for the tasks are: 89.2\%, 88.6\%, 78.1\%, 76.9\%, 74.8\%, 74.7\%, 67.3\%, 50\%, 50\%, 50\%. As a result of severe mislabelling, training was restricted to a containing 35-5,500 positive examples per task. Validation accuracies are all in the 88-99\% range, but as is argued in this report, this metric is inadequate when there is class imbalance. The recent discovery of the novel cost function means that it was only implemented for 3 of these tasks; these scores are therefore likely to increase thanks to this improvement alone.
}
\end{abstract}
