...
`CNN features off the shelf' by Razavian et al \cite{off-the-shelf} established state-of-the-art results by training linear SVMs on the features from the 7th, fully connected layer of OverFeat. One may wonder why `CNN features off the shelf' trains a linear SVM rather than a conventional softmax layer. 

A softmax output layer with two output units performs a logistic regression \cite{ML-book}.  	

(Razavian et al 2014) implements transfer learning by freezing backpropagation on all transferred layers, replacing the softmax output layer with an inner product layer containing as many output units, and training by minimising a hinge loss. Indeed, the SVM optimisation problem exposed in the literature review can be reformulated as the minimisation of the $L_2$ regularised hinge loss error function \cite{ML-book}, given by:

\begin{equation}
\mathcal{C}(y_i) = \sum \limits_{i=1}^{n} max(0, 1 - y_i t_i) + \lambda ||\textbf{w}||^2
\end{equation}

The $L_2$ regularisation component is $\lambda ||\textbf{w}||^2$, which penalises high weight values in order to reduce overfitting. The other component is the hinge loss itself, which can be more intuitively understood by looking at its graphical representation below (bearing in mind that with binary SVMs, target label values are \{1, -1\}).
