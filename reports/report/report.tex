...
Removal of the FC layer was simply for computation to be feasible on the GPU. This also meant using a smaller batch size. 
...
In fact, Bayesian softmax cross entropy by itself on AleNet architecture works best. Maximum per class accuracy is 78\%, which is truly impressive considering that this task was so difficult that transfer learning barely helped. Furthermore, learning was halted after 2000 iterations, before overfitting began, so more progress can be made.


\section{Final Results}

This presents best results (accuracy overall, on pos, on neg, sig level) for each binary classifier. \\
...
\begin{table}[h!]
   \centering
    ...
    \hline
    Clamp               & ~ 1,323  & ~     & ~    & ~ 0\%  & ~ 0\% \\
    Ground Sheet        & ~ 5,541  & ~     & ~    & ~ 0\%  & ~ 0\%    \\
    Hatch Markings      & ~ 3,793  & ~     & ~    & ~ 0\%  & ~ 0\%    \\
    Insertion Depth     & ~ 897    & ~     & ~    & ~ 0\%  & ~ 0\%      \\
    Soil Contam         & ~ 532    & ~ X   & ~ X  & ~ 78\%+  & ~ 0\%      \\
    Water Contam        & ~ 312    & ~ X   & ~ X  & ~ 0\%  & ~ 0\%    \\
    Scrape Zones        & ~ 2,545  & ~ X   & ~    & ~ 0\%  & ~ 0\%     \\
    Scraping or Peeling & ~ 292    & ~ X   & ~ X  & ~ 0\%  & ~ 0\%     \\
    Joint Misalignment  & ~ 35     & ~     & ~    & ~ 0\%  & ~ 0\% \\
    Fitting Proximity   & ~ 32     & ~ X   & ~    & ~ 0\%  & ~ 0\% \\
    \hline
    \end{tabular}
