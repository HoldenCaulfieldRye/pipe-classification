...
The first large-scale CNNs to be trained achieved encouraging results. LOOK THROUGH ENTIRE DOCUMENT, when talking about this training, the classification validation error achieved was 11\%, not 40\%! It may also be good to get a test error.
...
\subsubsection{Increase Test Error Precision}

By computing the test error on a large and unchanging sample of images, one obtains more precise estimates.
...
\paragraph{Volatility of Training Error}

Another striking aspect is how volatile the training error is, even when the test error is stable. This could be interpreted as the network's parameters having converged to values that make it good on certain batches, and bad on others. What is curious is that this would suggest that the contents of some batches are very different from one another (unless one considers the paper "Intriguing Properties of Neural Networks"). Therefore, it might be interesting to look at these batches in detail. \\
...
I think the determining characteristic is either mis-labelled proportion, or visual type of image.

\subsubsection{Alter Momentum}
...
\subsubsection{Reduce Learning Rate}

\subsubsection{Tackle Tight Bowl Zig-zagging}
