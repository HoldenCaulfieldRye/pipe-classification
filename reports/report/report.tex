...
The implementation quickly ran into unforeseen challenges which required experimentation to overcome. Therefore, this section is a mere overview of the hardware and software used to implement the design. How these were used is explained in the next section. \\

\subsection{Hardware}

CUDA-enabled GPU. How many teraflops? Which operations are parallelised? 
First a GTX GeForce 780 with 4GB RAM, then a Titan Black etc. 
...
As mentioned in the previously, a number of unexpected challenges arose throughout the implementation of the learning tasks. This section is therefore divided into task subsections, each of which contain challenge subsubsections, describing how they were dealt with.

\subsection{Task 1: Clamp Detection}
...
The first large-scale CNNs to be trained showed confusing results. LOOK THROUGH ENTIRE DOCUMENT, when talking about this training, the classification validation error achieved was 11\%, not 40\%! It may also be good to get a test error.
...
\subsection{Task 2: Clamp Detector}

This task arose as a result of data complexity, which is therefore featured as a first challenge below. \\
...
\subsubsection{Small Dataset: Transfer Learning}

Use transfer learning to make up for the lack of training examples. \\

\subsection{Task 3: Full Multi-Tagging Pipe Weld Classification Task}
