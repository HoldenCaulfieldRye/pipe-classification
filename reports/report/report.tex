...
Whereas with 88\% imbalance, it is possible to learn without transfer learning, 98\% imbalance makes it impossible, the model rapidly converges to a constant function. 
...
However, what remains a curiosity are the implications that this explanation has for the dynamics of learning: it would imply that when using cross entropy error, learning `focuses' on the majority class first, and afterwards focuses on the learning the minority class. This makes strategic sense, since there are more gains from improving on the majority class when using an error that is averaged across the min-batch. But it seems surprising that this could occur in the mechanics of gradient descent. Such work would perhaps make for interesting further research.
