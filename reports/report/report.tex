...
\begin{enumerate}
\item unlike every other neuron model, they are not symmetric.
...
The literature currently does not provide a mathematical explanation. 

"Rectified linear units improve restricted boltzmann machines" by Nair and Hinton (2010) provide... FILL IN!!

"ImageNet classification with deep convolutional neural networks" by Krizhevsky, Sutskever and Hinton (2012) \cite{krizhevsky} make the observations that $f(x) = tanh(x)$ or $f(x) = (1+ exp^{-x})^{-1}$ "saturating non-linearities" are slower to train than the $f(x) = max(0,x)$ "non-saturating non-linearity", and report an "accelerated ability to fit the training set", but do not provide any explanations. 

"Deep sparse rectifier networks" by Glorot, Bordes and Bengio (2013) focus on the performance of deep neural networks with ReLU activations (a.k.a\ deep sparse rectifier networks) by running experiments on supervised and unsupervised tasks as well as varied datasets. They make the important empirical discovery that "rectifying activation allows deep networks to achieve their best performance without unsupervised pre-training" given sufficient labelled training data. This plays an important part in the shift away from unsupervised pre-training to initialise the weights of a network, which would otherwise have been considered in the later stages of this project. Additionally, the authors address the fourth point raised above by explaining that the upward unboundedness of the ReLU can be controlled with an $L_1$ regulariser:  [!!!]. The authors use biological arguments to justify the superior performance of ReLU, namely that "studies on brain energy expense suggest that neurons encode information in a sparse and distributed way", 

However, they provide compelling intuitive arguments but NO mathematical explanation. However, they 
