...
After framing the task, this report provides an overview of supervised deep learning, followed by an in-depth explanation of the workings of a deep convolutional neural network, which despite its performance, gathers criticism for its tendency to be used as a black box. The rest of the report goes over the research that was carried out. The first is a simple mathematical analysis which proposes an explanation for the recent success of the Rectified Linear Unit as the chief activation function of CNNs\footnote{It came about as the result of seeking to understand, with limited assistance, aspects of training deep neural networks; it is uncertain whether existing research exceeds it.}. The next sections are applied and specific to the main task: understanding why learning failed during the first attempts; finding the optimal way to use transfer learning, exploring ways of tackling class imbalance -- including a novel cost function with a probabilistic interpretation which delivered up to 28 additional percentage points in classification performance; moving away from the common `AlexNet' CNN architecture to conserve spatial information; and final performance results across all tasks. \\

The bulk of research undertaken to make more general improvements in understanding deep learning and dealing with class imbalance, combined with time constraints, have restricted the time spent optimising each one of ControlPoint's tasks. For the moment, 3 tasks have per class accuracy in the 88-90\% range, 2 in the 75-80\% range, 1 at 62\%, and 4 have been found impossible to train. However, the recent discovery of the novel cost function means that it was only implemented for one of these tasks; these scores are therefore likely to increase significantly thanks to this straightforward improvement alone.
}
\end{abstract}
...
% Hey, I realise that with softmax we are placing no prior on the type of probability distribution. What if we did? e.g. if the NN were learning not to classify but to predict the probability of receiving 1,2,3,... letter this week conditional on date, number of friends, location etc, we might want to impose a Poisson constraint on the softmax. how would we do that? 
...
%particularly since (Zhou and Liu 2005) find that `almost all the techniques are effective on two-class tasks, while most are ineffective on multi-class tasks'.
...
\paragraph{Remove pooling and a fc layer}
...
\paragraph{Under-Sampling benchmark}
\begin{figure}
    \centering
    \begin{minipage}[b]{\textwidth}
      \begin{subfigure}{.5\textwidth} 
...
\paragraph{Remove Pooling, a FC layer and add Bayesian Cross Entropy}
...
\paragraph{Softmax Bayesian Cross Entropy and lower learning rate}
%     soil_contam/nopool                       TRAINING
\begin{figure}
    \centering
    \begin{minipage}[b]{\textwidth}
...
\begin{figure}
    ...
    \begin{minipage}[b]{\textwidth}
      \begin{subfigure}{.5\textwidth} 
        ...
        \includegraphics[scale=0.4]{images/plot_soil_contam_none_lr5_sbl.png}
        % Add \par\vspace*{20pt} here and...
        \caption{Batch size 96}\label{fig:2a}
      \end{subfigure}%
      \begin{subfigure}{.5\textwidth} 
        \centering
...
Despite how gradient noisiness decreased by raising the mini-batch size, what remains striking is that in figure ~\ref{f57}, the model represented on the right only has mini-batch size 96

%Furthermore, learning is halted after 2000 iterations (this was due to time constraints), before overfitting begins, so it seems clear from the plots that more progress can be made by resuming training. \\

\paragraph{Concluding Remarks}

A potential explanation for why the gains from the Bayesian cross entropy are much greater for this task than for clamp detection with 98\% imbalance is that imbalance is not as stark in this task, so the variance of the modulus of the gradient is lower, i.e.\ the gradient is less noisy. Since this issue could be addressed by ensuring equal class distribution at every mini-batch, the size of gains observed here could also potentially be seen with clampdet detection 98\% and other tasks with similar imbalances. \\

This section marks a failure for modifying the architecture of AlexNet, but it marks a victory for the Bayesian cross entropy, and encourages further empirical research into the benefits that this novel cost function can bring to training deep neural networks.


\section{Final Results}

We present best results for each binary classifier. The first three columns of figure ~\ref{f59} are indicators of the difficulty of the task: amount of available Bluebox examples, whether the task requires precise spatial information, whether the downsizing of the images to 256x256 prevented a human expert from carrying out classification. The last two columns are measures success: maximum average per class validation accuracy, and percentage of work that could be automated by setting the threshold so as to obtain 95\% accuracy on true positives. Note that as of this writing, Bayesian cross entropy was only tested on the soil contamination task, so in light of its success on that task as well as on clamp detection 98\%, scores are likely to increase significantly. \\
...
\begin{table}[h!]
   \centering
    ...
    Hatch Markings      & ~ 3,793  & ~     & ~    & ~ 76.9\%  & ~ 38\%    \\
    Insertion Depth     & ~ 897    & ~     & ~    & ~ 89.0\%  & ~ 72\%      \\
    Soil Contam         & ~ 532    & ~ X   & ~ X  & ~ 74.8\%  & ~ 32\%      \\
    Water Contam        & ~ 312    & ~ X   & ~ X  & ~ 50\%    & ~ 0\%    \\
    Scrape Zones        & ~ 2,545  & ~ X   & ~    & ~ 74.7\%  & ~ 32\%     \\
    Scraping or Peeling & ~ 292    & ~ X   & ~ X  & ~ 50\%    & ~ 0\%     \\
...
\begin{figure}
        ...
        ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc.
         ...
         \\
        \begin{subfigure}[b]{0.3\textwidth}
                \includegraphics[width=\textwidth]{images/plot_markings-fine_01-08-2014.png}
