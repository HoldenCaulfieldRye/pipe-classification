...
A meeting summarising then-current research results took place at ControlPoint's headquarters in mid-August. It was concluded that little more needed to be done to produce commercially deployable models, and talks for a licensing partnership have begun. Due to the belief that automation of human classification tasks and learning from high dimensional sensory data have the potential to deliver value in other industries such as construction, oil \& gas and medicine, a technology company has been founded as the result of this research project, with the aim of carrying out this vision. \\
}
\end{abstract}
...
The Google+ Deep Learning community was perhaps the 2nd most precious resource, after arXiv and before stackoverflow. It provided a large and steady stream of useful resources, comments and guidance. It seemed like a revolutionary way to be in touch with the world's leading researchers such as Prof Yann LeCun and Prof Yoshua Bengio. I owe particular thanks to Dr Soumith Chintala for his comments on transfer learning and Prof Francisco Zamora-Martinez for his on class imbalance.
...
\begin{figure}
    ...
    \begin{minipage}[b]{\textwidth}
      ...
      \begin{subfigure}{.5\textwidth} 
        \centering
        \includegraphics[scale=0.]{images/truly_confusing_2.jpg}
...
Cuda-Convnet is an open-source GPU implementation of a deep convolutional neural network written in CUDA C++ and python by Alex Krizhevsky to train AlexNet. It was therefore convenient to use it for the task, especially since  it had already been used for the MSc group project. However, a shortcoming with the API is that it expects data in the form of batches consisting of numpy arrays of stacked jpg values in matrix format with RGB values split across, and a dictionary of labels and metadata. Python programs were written to achieve this and extract training data from the log files and plotting it. By re-using code written during the group project, the additional code needing to be written was limited to approx.\ 400 lines. The hardware used for training the network was an nVidia GeForce GTX 780 with 4GB RAM. \\

\subsection{Experimentation}

Training occurred over the Redbox dataset only, to exclude domain change as a potential reason for weak performance if it were to occur. Time series for training and validation errors were extracted, but not the test error, since it serves no purpose in training the model. Note that training occurs by Stochastic
...
Training produced worryingly results: none of the training sessions showed any signs of parameter convergence, as the below plot of the training and validation errors over batch iterations can show. These results were confusing: backpropagation is guaranteed to converge \cite{DL-book}, and yet both training and validation errors seem stuck in volatile periodicity.
...
\item The dropout rate is too high: since neurons are randomly dropped at every iteration, a different model is tested every time; this is known to increase the volatility of the training error.
...
\item Class imbalance: there is not enough information about the "No Clamp" class to be able to learn features for them.
\item Mislabelled data: the images were not tagged correctly, so members of one class appear in the other and vice versa. \ldots
\item The error rates are not computed correctly \ldots
\end{itemize}

\paragraph{Increase Validation Error Precision}

If the set of images that the validation error is computed against varies from one iteration to the next, then the variations in validation error are not necessarily solely explainable by the changes in the model parameters; they may also be due to changes in the validation set. Secondly,  By computing the validation error on a larger sample of images, the measure becomes more precise. More importantly,  to another by making sure each validation error is computer over the same sample, one obtains measures which can truly be compared relative to one another.
