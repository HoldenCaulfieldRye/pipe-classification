...
As a test run, the most straightforward form of transfer learning was implemented on a network with the same architecture as was previously used, AlexNet.  This consists in taking the Caffe Reference Imagenet model, removing the 1000-output softmax layer (since the model was trained on the 1000 class ILSVRC 2012 challenge), and replacing it with a 2-output softmax layer with randomly initialised weights. The training data, as is the case for all subsequent experiments, consists in the Bluebox dataset. Below are the training results compared to training the same model, but with all weights randomly initialised.
