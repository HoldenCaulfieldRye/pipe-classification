...
\paragraph{Claim: transfer learning more tolerant of class imbalance}
...
\paragraph{Analytically: initialisation far from fake minima}
...
This section is a total mess, should probably erase. \\
...
You should properly benchmark 'CNN features off the shelf': use a linear SVM, so there is no kernel, just fitting hyperplanes, i.e. assume AlexNet feature space enables decent linear separation. Doesn't seem to be the case with this dataset! Maybe should use kernel trick on SVM? \\


\paragraph{Case 4: hatch markings, build on entire network or on conv layers only?}
...
15-08-2014 has 3 fc new layers on top conv[1-5] from AlexNet.
...
Results: 15 is significantly better, so information in fc layers is not as useful as raw filter information. In either case, model not expressive enough. However, on other extreme, 03 had full backprop and overfitted very quickly. There should be an optimal middle ground. 

Note these results are striking in light of 'CNN filters off the shelf', because there is such a large gap in performance 
...
Results are quite shocking: 13 has the new weights, within 3 iterations training loss attains its asymptote. It corresponds to a per class accuracy of 0.5, which suggests net is stuck in bad minimum. So even if you give it great filters, the convnet gets stuck in bad min. \\

14 has the alexnet weights, and it actually takes 2500 iterations to reach its asymptote. 

Notice that even with backprop on 3 fully connected layers, the network is not powerful enough to perfectly approximate the data (reach 0 training error). There is slack to make use of. Should this be by freeing backprop on 

Ok the comments above correspond to freeze5.5, I forgot to set blobs_lr to fc6 to 1 and 2. The nets are fucking up because fc6 has been initialised to 0 and can't learn its weights. \\
...
\clearpage
\section{Task 3: Bayesian Renormalisation}
