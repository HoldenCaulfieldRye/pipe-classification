...
Differentiable, symmetric, bounded activation functions that introduce a non-linearity smoothly over the entire input domain were of choice in deep feed-forward neural networks, until Rectified Linear Units were used in 2010 \cite{nair_hinton} and found to outperform their counterparts. This may come as a surprising result, since they are not fully differentiable nor symmetric, introduce non-linearity with an angular point, and most of all, are not bounded above. These points provide motivation to seek a mathematical explanation. \\

\subsection{Literature Review}

The literature currently does not provide a mathematical explanation backed by experimental evidence. 
...
"Deep sparse rectifier networks" by Glorot, Bordes and Bengio (2013) focus on the performance of supervised deep neural networks with ReLU activations, feobserve that ... explain that ...
