...
The literature \cite{imbalance} \cite{maloof} \cite{zhou} \cite{f-measure} has defined class imbalance as the situation where there exists a class of significantly smaller size than another, i.e.\ when the sample distribution of classes is significantly non-uniform. However, as will be seen later, this definition does not necessarily make class imbalance detrimental to learning. A definition for `dangerous class imbalance' will be given with the aim of being the largest set of conditions that are necessary for class imbalance to be detrimental for training classifiers with stochastic gradient descent and standard cost functions. \\

The literature on deep learning with class imbalance was found to be scarce. This report's definition may shed light on why: benchmark image classification datasets (such as CIFAR, MNIST and ImageNet) involve 10 to 1000 classes, so for a class to take up a majority of the training set is unlikely. Besides, if such a dataset did exist, it would arguably be considered unattractive the class imbalance `fake' minima never deliver less than 90\% error rate, which is unlikely to be an àttractive minimum. 

Zhou and Liu in `Training cost sensitive neural networks with methods addressing the class imbalance problem' (2005) \cite{zhou} experiment with \textbf{under-sampling}, textbf{over-sampling} and \textbf{threshold-moving} to deal with class imbalance, and find threshold-moving to perform best. \\
...
Class imbalance is defined in this report by the situation where the sample distribution of classes in the training set enables a classifier that is a constant function to perform significantly better than randomly outputting each class with the same probability. 

For example, given a 2-class training set with class distribution (0.9,0.1), a classifier which outputs the first class all the time obtains 90\% accuracy, whereas randomly outputting them uniformly obtains 50\%. 

It is not necessary for a class to occupy the vast majority of the training set: consider a 1000-class training set where one class takes up 20\%. A classifier which outputs this class all the time obtains 20\% top-1 accuracy, compared to 0.1\% for random uniform. 

It is not necessary for a class to occupy far more space than any other. Consider a 1000-class training set where 5 classes each take up 17\%. A classifier which always outputs these 5 as its top-5 obtains 85\% top-5 accuracy, which is higher even than the score of (Krizhevsky et al 2012) at ILSVRC 2012 \cite{krizhevsky}.

It is not sufficient for a class to occupy far less space than any other. Consider a 12-class training set where 11 take up 9\% each and one takes up 1\%. 

Others \cite{f-measure} \cite{imbalance} have defined class imbalance more lightly and intuitively as the situation where there exists a class of significantly smaller size than another, i.e.\ when the sample distribution of classes is significantly non-uniform. However, as it will be observed and proved later, this report's definition is the largest set of conditions that are necessary for the detrimental phenomenon of interest to occur when training classifiers with stochastic gradient descent and standard cost functions. \\
...
Under-sampling rebalances the training set by removing majority class examples. The brute force approach is to randomly remove these cases and train on the remaining set. However, (Zhou and Liu 2005) \cite{zhou} describe an algorithm for removing selectively. It was not implemented during this project mainly due to its time complexity, which is analysed below. 

\begin{enumerate}
\item Obtain a perfectly balanced training set by randomly setting aside non-minority class examples.
\item Train a net on the set.
...
To the best of our understanding, it is unclear from the paper whether 
...
\begin{figure}
    ...
    \begin{minipage}[b]{\textwidth}
      ...
      \begin{subfigure}{.5\textwidth} 
        \centering
        %\includegraphics[scale=0.4]{images/plot_clampdetCI98_none_bs128_lr5.png}
        % ...here to lower the subcaptions and...
        \caption{learning rate 0.00001}\label{fig:2b}
      \end{subfigure} \par \vspace*{20pt} % ...remove them from here
      \caption{98\% imbalance, different learning rates}\label{fig:2}
...
Different class imbalance definition, specifically tailored to training classifiers with stochastic gradient descent.
