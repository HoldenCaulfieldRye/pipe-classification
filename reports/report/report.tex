...
Training occurred over the Redbox dataset only, to exclude domain change as a potential reason for weak performance if it were to occur. Time series for training and validation errors were extracted, but not the test error, since it serves no purpose in training the model.
...
By computing the validation error on a larger sample of images, the measure becomes more precise. More importantly, by making sure each validation error is computer over the same sample, one obtains measures which can truly be compared relative to one another.
