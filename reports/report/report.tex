...
\begin{figure}[!h]
	...
	\caption{Test Run Training Results\footnote{the Y axis label should read logprob error, not \% error rate}}
\end{figure}
...
\begin{figure}[h!]
	...
	\caption{Test Run with validation errors computed over an unchanging set\footnote{the Y axis label should read logprob error, not \% error rate}}
\end{figure}

With better validation error measures, one notices that the validation error is flat throughout most of training, confirming that nothing is learned during the corresponding time interval. There is some movement during the first thousand iterations, suggesting in fact that backpropagation converged within 1000 iterations, a small amount relative to comparable tasks \cite{decaf} \cite{fergus_tutorial} cite{transfer-learning} \cite{caffe-website}. One may be tempted to conclude that the model is able to learn only very little.
...
An intriguing aspect is that the 0.4 validation (logprob) error that the model converges to corresponds to a 10\%  error rate, which is the proportion of images that belong to the majority class "Clamp Detected". Therefore, an explanation for why the network converges relatively fast to logprob 0.4 minimum, and stays there, without ever overfitting, is because class imbalance has introduced a `fake' minimum in a `corner' of the error-parameter space. This location in the parameter space corresponds to the model being a constant function that always outputs $(1, 0, 0)$, where the first entry is the probability score for the majority class `Clamp Detected'. This minimum is very hard to get out of because it is deep: it enables the model to score a 10\% error rate, which might be better than a number of other local minima. It is far away from where the `real' minima are because the `real' minima are in a region of the parameter space that corresponds to the network having learned meaningful features (e.g.\ in the case of the convolutional layers, actual Gabor filters). \\
...
\begin{table}[h]
   \centering
    ...
    \hline
    Training Error   & ~  "Clamps Detected"  & ~ "No Clamps"        & ~ "Photo Does Not Show Enough Of Clamps" \\ \hline
...
This local minimum is `fake' because, with perfect class imbalance, it would not exist. Or at least, it would correspond to a $\frac{1}{K}$ error rate (where $k$ is the number of classes), the neighbourhood defined by all 
