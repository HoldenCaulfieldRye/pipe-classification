...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_clampdet_conv1.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_clampdet_none_reinit.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_clampdet_linSVM.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
A fixed architecture is kept throughout the experiments: backprop on all layers, all weights transferred. Note that this is no longer SGD with fitting proximity because dataset is so small, we are using the entire training set every time. \\

Test interval set to 1, because more important to follow whether or how quickly net goes into bad local minimum. \\

\subsubsection{Test Run, Under-Sampling}
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=•]{•}=0.5]{images/plot_fitting_proximity_tl_wout.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[scale=•]{•}=0.5]{images/plot_fitting_proximity_us_tl_wout.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
Notice how under-sampling makes overfit take place so much sooner. This begs for other class imbalance tacklign tricks to be combined with under-sampling, in order to limit the extent to which we under-sample, in order to fragilise robustness as little as possibe, so that we can use more expressive models, i.e.\ enable backprop on more layers. \\
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_fitting_proximity_us_none.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}

We can see from above that the good minimum is at ? . Therefore, to evaluate hypothesis expressed above, we can under-sample with a target bad min below this good min ('fit_prox_usBelow'), another slightly above ('fit_prox_usAbove'), and do transfer learning with no reinit for both. If hypothesis is correct, then network should settle in bad min for former, but not for latter. \\

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_fitting_proximity_usAbove_none.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}

\begin{figure}[h!]
	\centering
	\includegraphics[scale=0.5]{images/plot_fitting_proximity_usBelow_none.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
Maybe a separating hyperplane is less sensitive to small datasets than logistic regression? \\
...
\begin{figure}[h!]
	\centering
	\includegraphics[scale=•]{•}=0.5]{images/plot_clampdet_us_none_reinit.png}
	\caption{Clamp Detection, Full Backpropagation}
\end{figure}
...
Get Razvan's Bayesian interpretation of what the net learns and therefore why the threshold has a nice probabilistic interpretation. \\
